...
class GEARS_Model(torch.nn.Module):
    ...
    def __init__(self, args):
        ...
        self.mean_expression = args['mean_expression']
    ...
    def forward(self, data, return_h = False):
        x, pert_idx = data.x, data.pert_idx

        if self.mean_pert:
            num_graphs = len(data.batch.unique())
            return self.mean_expression.repeat(num_graphs, 1)
        ...
        if self.no_perturb:
            ...
            if return_h == 'before_gene_specific_layer1':
                return out
            ...
            if return_h == 'cross_gene_embed':
                return cross_gene_embed
            
            if return_h == 'cross_gene_out':
                return cross_gene_out
            
            cross_gene_out = cross_gene_out * self.indv_w2
            cross_gene_out = torch.sum(cross_gene_out, axis=2)
            out = cross_gene_out + self.indv_b2
            if return_h == 'diff_effect':
                return out
